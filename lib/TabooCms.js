...
class TabooCms extends EventEmitter {
  ...
  getModel(moduleString) {
    return CmsHelper.getModuleModel(moduleString);
  }
  ...
  parseApiQuery(request) {
    return ApiHelper.parseQueryParams(request);
  }
  ...
  setupAppModules() {
    const modules = CmsHelper.getAllModulesPaths();
    modules.map(module => {
      let moduleConfig;
      try {
        moduleConfig = require(module.configFile);
        ...
        if (moduleConfig.routes) {
          this.setupRoutes(moduleConfig, module.path);
        }
      } catch (e) {
        Logger.error(e);
      }
  ...
  setupDb() {
    _.each(this.app.config.db.connections, (config, name) => {
      if (config && config.driver === 'mongodb') {
        this.app.dbConnections[name] = new MongoDbAdapter(config);
        this.app.dbConnections[name].connect(
          config.options,
          this.setupModels.bind(this)
        );
      }
    });
  }
  ...
  setupModels() {
    _.each(this.app.modules, module => {
      if (module.config.models && _.size(module.config.models) > 0) {
        _.each(module.config.models, (model, modelName) => {
          // TODO implement models pre and post hooks
          this.app.dbConnections[model.connection].setupModel(module, model, modelName);
        });
      }
    });
  }
  ...
  setupRoutes(moduleConfig, modulePath) {
    ...
    if (moduleConfig && moduleConfig.routes && moduleConfig.controller) {
      moduleConfig.routes.map(route => {
        this.app.router[route.method.toLowerCase()](route.path, async (ctx, next) => {
          ...
          return await route.action.call(moduleConfig.controller, ctx, next);
        });
      });
    }
  }
  ...
  async getPage(ctx) {
    ...
    if (module && module.route && module.route.action && module.route.action.name && module.path) {
      view = `${module.route.action.name}.${server.modules.viewsExtension}`;
