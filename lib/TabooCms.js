...
class TabooCms extends EventEmitter {
  ...
  setupRoutes(routes) {
    routes.map(route => {
      if (route.path === '*') {
        // Wildcard for any route will be setup at the end in setupServerResponse
        this.unmatchedRoutes = route;
      } else {
        this.app.router[route.method.toLowerCase()](...CmsHelper.getRouterRouteArgs(route));
      }
    });
  }
  ...
  setupServerResponse() {
    this.app.server.use(async (ctx, next) => {
      ...
      if (ctx.err) {
        ...
        ctx.body = response;
      } else if (this.unmatchedRoutes) {
        CmsHelper.setupRoutePolicies(policies, this.unmatchedRoutes);
        await this.unmatchedRoutes.action(this.unmatchedRoutes, policies, ctx, next);
      } else {
        await next();
      }
    });
  ...
  async getServerResponse(ctx) {
    const page = await this.getPage(ctx, ctx.taboo.module, ctx.data._view);
    const layout = await this.getLayout(ctx, ctx.data._layout);
