const mongoose = require('mongoose');
const _ = require('lodash');
const Logger = require('./Logger');
const DbAdapter = require('./DbAdapter');

class MongoDbAdapter extends DbAdapter {
  constructor(config) {
    super(config);
  }

  async connect(options) {
    let connectionString = 'mongodb://';
    let db;

    if (this.config.user) {
      connectionString += this.config.user;
    }
    if (this.config.password) {
      connectionString += `:${this.config.password}`;
    }
    if (this.config.user) {
      connectionString += '@';
    }
    connectionString += `${this.config.host}:${this.config.port}/${this.config.database}`;

    mongoose.connect(
      connectionString,
      options
    );

    db = mongoose.connection;

    db.on('error', err => {
      Logger.error(err);
    });

    await db.once('open', () => {
      Logger.info(
        `Successfully connected to MongoDB at: ${this.config.host}:${this.config.port}/${this.config.database}`
      );
    });
    return db;
  }

  setupModel(module, modelConfig, modelName) {
    if (!modelConfig.schema) {
      throw new Error('Model must have "schema" specified');
    }
    if (!modelConfig.connection) {
      throw new Error('Model must have "connection" key name value');
    }
    const schema = new mongoose.Schema(modelConfig.schema);

    if (modelConfig.afterSchemaCreate && _.isFunction(modelConfig.afterSchemaCreate)) {
      modelConfig.afterSchemaCreate(schema);
    }
    if (!module.models) {
      module.models = {};
    }
    module.models[modelName] = mongoose.model(modelName, schema);
    if (modelConfig.afterModelCreate && _.isFunction(modelConfig.afterModelCreate)) {
      modelConfig.afterModelCreate(module.models[modelName], schema);
    }
  }
}

module.exports = MongoDbAdapter;
