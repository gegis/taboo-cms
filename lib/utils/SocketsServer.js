...
class SocketsServer {
  ...
  start(server) {
    if (this.config.sockets.port) {
      Logger.info(
        `Starting sockets server on port ${this.config.sockets.port}`
      );
  ...
  onClientJoin(socket, room) {
    if (this.config.sockets.rooms.indexOf(room) !== -1) {
      socket.join(room);
      this.io
        .to(room)
        .emit('join', { clients: this.getClientsCountInRoom(room) });
  ...
  getClientsCountInRoom(room) {
    if (
      Object.prototype.hasOwnProperty.call(this.io.sockets.adapter.rooms, room)
    ) {
