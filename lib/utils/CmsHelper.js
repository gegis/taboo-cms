...
class CmsHelper {
  ...
  getLocalesArray() {
    ...
    const localesArray = [];
    let item;
    for (let locale in this.app.locales) {
      if (locales.indexOf(locale) === -1) {
        locales.push(locale);
      }
      for (let key in this.app.locales[locale]) {
        if (!helper.hasOwnProperty(key)) {
          helper[key] = {};
        }
        helper[key][locale] = this.app.locales[locale][key];
      }
    }
    ...
    for (let key in helper) {
      ...
      item.key = key;
      locales.map(locale => {
        item[locale] = this.app.locales[locale][key];
      });
  ...
  isAllowed(target, resource) {
    this.isAllowedImplementation(target, resource);
  }
}
